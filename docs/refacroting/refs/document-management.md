...
- **保全期間の扱い**: 14日間参照がなければ Archive → Delete の検討に移る（参照ログを根拠に判断）、アラートを出して確認する。
...
- 参照したら、以下の表に **1行追記**する
- 更新タイミング:
  - ドキュメントを新規作成/更新/参照した日のうちに追記する
  - 週1回（例: 金曜）に棚卸しして、Archive/削除候補の移動を検討する
...
## 4.1. 運用ルール：不要ファイルは「即削除しない」で、あとでまとめて消す

このリポジトリでは、差分肥大と迷子を防ぐために **不要ファイルの削除を“即実行”しません**。  
作業中に見つけた不要物は、次の順序で扱います。

1. **見つけたら記録**: 「削除候補リスト」に追加（理由・代替先をセットで）
2. **参照の可視化**: 参照ログ（Document Ledger）に、判断の根拠として残す
3. **まとめて削除**: ユースケース単位の区切り（例: 背骨UC完了）で一括削除する

例: AWS未運用の情報（古いデプロイ手順/古いworkflow参照）は、まず候補化してからまとめて消す。
