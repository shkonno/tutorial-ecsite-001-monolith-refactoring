...
**あなたの役割**: 会話履歴を分析し、`.cursorrules`または`.cursor/rules/`配下のルールファイルに追記すべき新しいルールやパターンを提案すること。

**重要**: 会話の要約や説明ではなく、「Cursorルールファイルに追記する内容の提案」を出力してください。
...
トリガーに引っかかった場合：
```markdown
会話履歴を分析しました。以下の内容をCursorルールファイルに追記しませんか？

追記した方がよさそうであれば、「この内容をCursorルールファイルに追記してください」のように指示してください。
...
トリガーに引っかからなかった場合：
```markdown
会話履歴を分析しました。Cursorルールファイルに追記すべき新しい内容は見つかりませんでした。
```
...
### 提案しない（×）
- 一時的な判断や個別ケースの対応
- 既にCursorルールファイルに記載されている内容
- 個人の好みや一時的な試行
- 不明確または曖昧な指示

## 実行後のアクション
1. 提案内容をレビュー
2. 適切と判断した内容をCursorルールファイルに追記することをAIアシスタントに指示
3. コード変更と同じPRに含める
